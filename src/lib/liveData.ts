...
/**
 * Sponsor rows, live from the sponsor sheet when connected. Until then (or
 * whenever the sheet is unreachable) it falls back to the bundled sponsor
 * config — so the flagship's real sponsors always show, and connecting a sheet
 * only ever upgrades them.
 */
...
/**
 * Synchronous best-effort sponsors for non-React callers (the PDF export):
 * the last good copy the live hooks cached on this phone, or the bundled
 * config when no sheet is connected. Keeps the printed scorecard in step with
 * what players saw on screen.
 */
export function currentSponsors(courseId: string = defaultCourse.id): Sponsor[] {
  const fallback = getSponsors(courseId);
  if (sheetsConfig.sponsorSheetId.length === 0) return fallback;
  const cached = readCache<Sponsor[]>(SPONSOR_CACHE_KEY);
