...
2. *(Coming next milestone)* **the course data file** — the Osceola holes will
   live in one clearly-labeled config file so dropping in the real hole data
   from the owner's worksheet is a one-file change.
...
2. ⬜ Course data model + placeholder Osceola course from config.
